import type { PreparedImages } from '@/services/official-image-preparation';

export const FIELD_BOOK_KEY = 'reprise.fieldbook.captures.v1';
export const FIELD_BOOK_LIMIT = 50;

export type CapturePreparation = PreparedImages;
...
type LegacyCapture = Omit<Partial<SavedCapture>, 'preparation'> & {
  frame?: number;
  preparation?: {
    current?: boolean | PreparedImages['current'];
    reference?: boolean | PreparedImages['reference'];
  };
};
...
function normalize(raw: unknown): SavedCapture | undefined {
  ...
  const normalizePreparation = (
    value: boolean | PreparedImages['current'] | undefined,
    readyFallback = false,
  ): PreparedImages['current'] =>
    typeof value === 'object' && value !== null
      ? { ready: value.ready === true, ...(value.error ? { error: value.error } : {}) }
      : { ready: value === true || readyFallback };
  return {
    schemaVersion: 2,
    id: capture.id,
    ...
    coordinate:
      isFiniteNumber(capture.coordinate?.latitude) && isFiniteNumber(capture.coordinate?.longitude)
        ...
        : undefined,
    locationPrecision:
      capture.locationPrecision === 'precise' || capture.locationPrecision === 'approximate'
        ? capture.locationPrecision
        : undefined,
    preparation: {
      current: normalizePreparation(preparation?.current, Boolean(capture.assetId)),
      reference: normalizePreparation(preparation?.reference),
    },
...
export function createFieldbookStore({ storage, files, now = () => new Date() }: StoreDependencies) {
  ...
  const save = async (input: NewCapture): Promise<SavedCapture> => {
    ...
    const capture: SavedCapture = {
      ...
      id,
      imageUri: durableUri,
      preparation: input.preparation ?? { current: { ready: false }, reference: { ready: false } },
